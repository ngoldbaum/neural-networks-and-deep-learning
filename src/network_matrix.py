...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for j in xrange(epochs):
            ...
            inputs = [np.array([i[0] for i in mb]) for mb in mini_batches]
            classifications = [np.array([i[1] for i in mb]) for mb in mini_batches]
    ...
    def backprop(self, inp, classification):
        ...
        delta = self.cost_derivative(activations[-1], classification) * \
            sigmoid_prime(zs[-1])
        nabla_b[-1] = delta
        nabla_w[-1] = np.matmul(delta, np.swapaxes(activations[-2], -2, -1))
